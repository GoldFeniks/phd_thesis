...
\begin{document}
	...
	\end{gather}
	\par В \textbf{разделе 2.3} подробно рассматриваются аппроксимация псевдодифференциального оператора и дискретизация уравнения \eqref{eq::PDMPE} по координате $x$ с использованием аппроксимации Паде и метода Split-step Pad\'e. Последний заключается в формальном решении уравнения на равномерной сетке $\Delta x=h$ в виде 
	\begin{equation}
    ...
    \begin{figure}[h]
    	\begin{minipage}[h][0.33\textheight]{0.4\textwidth}
    		...
    		\vfill
    		\captionof{figure}{Схематическое изображение подводного каньона\label{fig::wedge}}
    	\end{minipage}
    	\hfill
    	\begin{minipage}[h][0.33\textheight]{0.5\textwidth}
    		...
    		\vfill
    		\captionof{figure}{Акустическое поле (в дБ отн. 1 м.) в мелком море с подводным клином $z=30\ \text{м.}$\label{fig::wedge_field}}
    	\end{minipage}
    \end{figure}
    \begin{figure}[h]
    	\centering
    ...
    \end{figure}
    \FloatBarrier
    \par Далее было проведено сравнение горизонтальных лучей, вычисленных с использованием программы AMPLE \cite{ample}, а также лучей, полученных из закона Снеллиуса в виде криволинейного интеграла второго рода вдоль кривой $l: y\pa{s}$ и выпущенных под углом $\psi$

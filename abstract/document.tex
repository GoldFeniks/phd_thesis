...
\begin{document}
	...
	\end{equation}
	где $\bm{\Psi}_j^{n,q}=\pa{\mathcal{A}_j^{n,q},\mathcal{B}_{j,1}^{n,q},\dots,\mathcal{B}_{j,1}^{n,q}}^T\in\mathbb{C}^{p+1}$. Вычисление коэффициентов $D_j$ подробно описано в тексте работы.
	...
	\begin{gather}
		...
		S_j\pa{x_0,y\pa{l,\alpha}}=\int\limits_0^ln_j\pa{y\pa{l,\alpha}}\dd\ell\,,\\
		M_j\pa{x_0,y\pa{l,\alpha}}=\frac{M_{j,0}}{n_j\pa{y\pa{l,\alpha}}}\sqrt{\frac{\cos\alpha}{}}
		
	\end{gather}
	\newpage
	\printbibliography[title=Цитированная литература,notkeyword={myarticles},notkeyword={myproceedings}]
	\printbibliography[title=Список публикаций,filter=mine]
\end{document}

...
\begin{document}
    \section{Математические методы\label{seq::math}}
        \subsection{Модовое представление звукового поля точечного источника\label{sec::sound_modes}}
            ...
            \end{equation}
            где $N_m=\left\lfloor\frac{\omega H}{\pi c}\right\rfloor$, $c\pa{x,y,z}$ --- скорость звука, $\omega=2\pi f$ --- циклическая частота, $\beta\pa{x,y,z}$ --- коэффициент затухания звука, $\eta=\nicefrac{1}{40\pi\log_{10}e}$. Для существования и единственности решения требуется также поставить условия излучения Свешникова \cite{sveshnikov,alekseev}
            ...
            \end{equation}
            В дальнейшем удовлетворение этому условию следует из построения модового параболического уравнения и численный схемы его решения: учёт только исходящих волн по $x$ (см. \ref{seq::MPE}) и подавление отражения по $y$ (см. \ref{seq::boundary_conditions}).
        ...
        \subsection{Уровень звукового воздействия\label{sec::SEL}}
            ...
            \begin{equation}
                ...
                \end{gathered}
            \end{equation}
        \subsection{Колебательные скорость и ускорение}

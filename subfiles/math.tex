...
\begin{document}
    \section{Математические методы\label{seq::math}}
        \subsection{Модовое представление звукового поля точечного источника\label{sec::sound_modes}}
            ...
            \begin{align}
                ...
                p\bigr|_{z=H}&=0
            \end{align}
            где $\rho\pa{x,y,z}$ --- плотность среды, $H$ -- нижняя граница расчётной области, $\mathbf{n}$ --- нормаль к границам разделов слоёв среды, $h_i,i=\overline{1,N_h}$ --- глубины этих разделов. Коэффициент $K\pa{x,y,z}$ определяется как
            ...
            \end{equation}
            где $N_m=\left\lfloor\frac{\omega H}{\pi c_\infty}\right\rfloor$, $r=\sqrt{x^2+y^2}$, а $p_j\pa{r}$ являются коэффициентами разложения решения $p\pa{x,y,z}$ уравнения Гельмгольца по собственным функциям $\phi_j$ оператора Штурма-Лиувилля $\pa{\Delta+K^2}$
            \begin{equation}
                p\pa{x,y,z}=\sum\limits_{j=1}^\infty p_j\pa{r}\phi_j\pa{z}\,.
            \end{equation}
            Рассмотрим сначала задачу в горизонтально однородной среде, то есть\\ $\rho\pa{x,y,z}=\rho\pa{z}$, $c\pa{x,y,z}=c\pa{z}$ и $\beta\pa{x,y,z}=\beta\pa{z}$, при этом также будем считать плотность среды $\rho\pa{z}$ кусочно-постоянной функцией по слоям среды
            \begin{equation}
            ...
            \end{equation}
            В дальнейшем удовлетворение этому условию следует из построения модового параболического уравнения и численной схемы его решения: учёт только исходящих волн по $x$ (см. \ref{seq::MPE}) и подавление отражения по $y$ (см. \ref{seq::boundary_conditions}).
        \subsection{Модовые параболические уравнения\label{seq::MPE}}
            ...
            \end{equation}
        \subsection{Аппроксимация Паде\label{seq::pade}}
        ...
        \subsection{Граничные условия\label{seq::boundary_conditions}}
            ...
            \subsubsection{Граничные условия прозрачности}
                ...
                \end{equation}
            \subsubsection{$\mathcal{Z}$-преобразование\label{seq::ztransform}}
        ...
        \subsection{Уровень звукового воздействия\label{sec::SEL}}
            ...
            \begin{equation}
                ...
                \end{gathered}
            \end{equation}
        \subsection{Колебательные скорость и ускорение\label{seq::particle_acceleration}}

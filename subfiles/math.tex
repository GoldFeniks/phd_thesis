...
\begin{document}
	\section{Математические методы}
		\subsection{Модовое представление звукового поля точечного источника\label{sec::sound_modes}}
			...
			\begin{equation}
				K\pa{x,y,z}=\frac{\omega}{c\pa{x,y,z}}\pa{1+i\nu\beta\pa{x,y,z}}\,,
			\end{equation}
        ...
        \subsection{Граничные условия}
            ...
            \subsubsection{Прозрачные граничные условия}
            	\par Граничные условия, позволяющие предотвратить отражение от произвольной границы называются прозрачными граничными условиями. Для узкоугольного параболического уравнения они были впервые получены в непрерывной форме Башкаковым и Поповым \cite{baskakov1991} и Маркусом \cite{marcus1991}. В дальнейшем Попвым также были получены условия для широкоугольного параболического уравнения в самой простой форме. Полностью дискретные условия для узкоугольного параболического уравнения пригодные для использования в численном моделировании были получены Арнольдом и Эрхардтом \cite{ehrhardt2002,xavier2008}, которые в дальнейшем были расширены для широкоугольных параболических уравнений \cite{arnold1998}.
            	...
            	\end{equation}
            	Решение такой системы может быть найдено с использованием $\mathcal{Z}$-преобразования по эволюционной координате $x$
            	\begin{equation}
            	...
            	\end{equation}
            	Обозначим $\hat{\bm{\Psi}}_j^q=\pa{\hat{\mathcal{A}}_j^q,\hat{\mathcal{B}}_{j,1}^q,\dots,\hat{\mathcal{B}}_{j,p}^q}^T\in\mathbb{C}^{p+1}$, добавим к системе $Z$-преобразование уравнения \eqref{eq::BtoA}
            	\begin{equation}
            	...
            	\begin{equation}
            		...
            		\end{pmatrix}\,,
            	\end{equation}
            	где $D_q^-\hat{\bm{\Psi}}_j^q=\nicefrac{\hat{\bm{\Psi}}_j^q-\hat{\bm{\Psi}}_j^{q-1}}{\delta}$ и $D_q^+\hat{\bm{\Psi}}=\nicefrac{\hat{\bm{\Psi}}_j^{q+1}-\hat{\bm{\Psi}}_j^q}{\delta}$ -- конечные разности назад и вперёд соответственно. Решение этой системы может быть записано в следующем виде
            	\begin{equation}
            		\begin{pmatrix}
            			\hat{\bm{\Psi}}_j^{q+1}\\
            			\hat{\bm{\xi}}_j^{q+1}

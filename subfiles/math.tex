...
\begin{document}
    \section{Математические методы\label{sec::math}}
        \par Текущая глава посвящена описанию различных математических методов, предназначенных для моделирования распространения звука в волноводах мелкого моря, а также описанию различных задач, решение которых может быть выполнено с использованием разрабатываемой модели. \niceref{sec::sound_modes}{Разделы} и \ref{sec::MPE} посвящены краткому описанию метода нормальных мод в акустике. В \niceref{sec::pade}{Разделе} рассматривается использование аппроксимации Паде \cite{avilov, collins} для приближения оператора квадратного корня и пропагатора псевдодиффиренциального модового параболического уравнения, а затем в \niceref{sec::lj}{Разделе} рассматривается дискретизация дифференциального оператора по поперечной горизонтальной координате и приводится численная схема решения псевдодифферециального модового параболического уравнения. Далее представлены особенности его решения с использованием представленной численной схемы. В \niceref{sec::boundary_conditions}{Разделе} рассматриваются два способа искусственного ошраничения вычислительной области: согласованные поглощающие слои и граничные условия прозрачности. \niceref{sec::ray_theory}{Раздел} посвящён краткому описанию лучевой теории для уравнения горизонтальной рефракции, которая затем используется для постановки лучевых начальных условий в \niceref{sec::ray_starters}{Разделе}, моделирующих точечный источник. Последними в \niceref{sec::impulse}{Разделах}, \ref{sec::SEL} и \ref{sec::particle_acceleration} приводятся задачи, связанные с моделированием распространения звука: расчёт временных рядов в точках приёма, расчёт полей уровней звуковой экспозиции, колебательных скорости и ускорения.
        \subsection{Модовое представление звукового поля точечного источника\label{sec::sound_modes}}
            ...
            \begin{equation}\label{eq::3DH}
                \pa{\rho\pa{x,y,z}\nabla\pa{\frac{1}{\rho\pa{x,y,z}}\nabla} + K^2\pa{x,y,z}}p\pa{x,y,z}=-\delta\pa{x}\delta\pa{y}\delta\pa{z-z_s}\,,
            \end{equation}
            а также граничными условиями и условиями на границах раздела сред вида
            \begin{align}
                p\bigr|_{z=0}&=0\,,\\
                p\bigr|_{z=h_i^{-}}&=p\bigr|_{z=h_i^{+}}\,,\\
                \frac{1}{\rho}\frac{\partial p}{\partial\mathbf{n}}\biggr|_{z=h_i^{-}}&=\frac{1}{\rho}\frac{\partial p}{\partial\mathbf{n}}\biggr|_{z=h_i^{+}}\,,\\
                p\bigr|_{z=H}&=0
            \end{align}
            где $\rho\pa{x,y,z}$ --- плотность среды, $H$ -- нижняя граница расчётной области, $\mathbf{n}$ --- нормаль к границам разделов слоёв среды, $h_i,i=\overline{1,N_h}$ --- глубины этих разделов. Коэффициент $K\pa{x,y,z}$ представляет собой (комплексное) волновое число среды и определяется как
            \begin{equation}\label{eq::K}
                K\pa{x,y,z}=\frac{\omega}{c\pa{x,y,z}}\pa{1+i\eta\beta\pa{x,y,z}}\,,
            \end{equation}
            где $c\pa{x,y,z}$ --- скорость звука, $\omega=2\pi f$ --- циклическая частота, $\beta\pa{x,y,z}$ --- коэффициент затухания звука, $\eta=\nicefrac{1}{40\pi\log_{10}e}$. Для существования и единственности решения требуется также поставить условия излучения Свешникова \cite{sveshnikov,alekseev}
            \begin{equation}\label{eq::radiation_condition}
                \lim\limits_{r\to\infty}\sqrt{r}\pa{\frac{\partial}{\partial r}-i\sqrt{\frac{\omega}{c\pa{r}}}}p_j\pa{r}=0\,,\quad j=\overline{1,N_m}\,,
            \end{equation}
            где $N_m=\left\lfloor\frac{\omega H}{\pi c_\infty}\right\rfloor$, $r=\sqrt{x^2+y^2}$, а $p_j\pa{r}$ являются коэффициентами разложения решения $p\pa{x,y,z}$ уравнения Гельмгольца по собственным функциям $\phi_j$ оператора Штурма-Лиувилля $\pa{\frac{\partial^2}{\partial x^2}+\frac{\partial}{\partial y^2}+K^2}$ на бесконечности (при $r\mapsto\infty$), где волновод считается регулярным
            \begin{equation}
                p\pa{x,y,z}=\sum\limits_{j=1}^\infty p_j\pa{r}\phi_j\pa{z}\,.
            \end{equation}
            Рассмотрим сначала задачу в горизонтально однородной среде (регулярном волноводе), то есть\\ $\rho\pa{x,y,z}=\rho\pa{z}$, $c\pa{x,y,z}=c\pa{z}$ и $\beta\pa{x,y,z}=\beta\pa{z}$. При этом также будем считать плотность среды $\rho\pa{z}$ кусочно-постоянной функцией по слоям
            \begin{equation}
                \rho\pa{z}=\begin{dcases}
                    \rho_0,&0\leqslant z<h_1\\
                    \rho_i,&h_i<z<h_{i+1},\quad i=\overline{1,N_h-1},\\
                    \rho_{N_h},&h_{N_h}<z\leqslant H\,.
                \end{dcases}
            \end{equation}
            Используя технику разделения переменных и представляя звуковое поле в виде
            \begin{equation}
            ...
            \begin{equation}
                \begin{dcases}
                    ...
                    \varphi\bigr|_{z=0}=0\,,\\
                    \varphi\bigr|_{z=h_i^{-}}=\varphi\bigr|_{z=h_i^{+}}\,,\\
                    \frac{1}{\rho}\frac{\dd \varphi}{\dd z}\bigr|_{z=h_i^{-}}=\frac{1}{\rho}\frac{\dd \varphi}{\dd z}\bigr|_{z=h_i^{+}}\,,\\
                    \varphi\bigr|_{z=H}=0\,.
                \end{dcases}
                \label{eq::SLP}
            \end{equation}
            ...
            \begin{enumerate}
                ...
                \end{equation}
                \item функции $\varphi_j\pa{z}$ имеют ровно $j+1$ корень на отрезке $\left[0,H\right]$,
            ...
            \begin{multline}
                \sum\limits_{j=1}^\infty\left(\frac{\partial^2 A_j\pa{x,y}}{\partial x^2}\varphi_j\pa{z,x,y}+2\frac{\partial A_j\pa{x,y}}{\partial x}\frac{\partial\varphi_j\pa{z,x,y}}{\partial x}+\frac{\partial^2\varphi_j\pa{z,x,y}}{\partial x^2}A_j\pa{x,y}\right.+\\
                \frac{\partial^2 A_j\pa{x,y}}{\partial y^2}\varphi_j\pa{z,x,y}+2\frac{\partial A_j\pa{x,y}}{\partial y}\frac{\partial\varphi_j\pa{z,x,y}}{\partial y}+\frac{\partial^2\varphi_j\pa{z,x,y}}{\partial y^2}A_j\pa{x,y}+\\
            ...
            \end{equation}
            Такое уравнение принято называть уравнением горизонтальной рефракции. Для того, чтобы получить условия излучения, предположим также, что на некотором достаточно большом расстоянии $r_{max}$ параметры среды становятся постоянными по горизонтальным координатам, то есть $k_j\pa{r}=k_{j,\infty}$ при $r>r_{max}$. Тогда условия излучения \eqref{eq::radiation_condition} преобразуются к следующему виду \cite{dhia2009}
            \begin{equation}
                \varlimsup\limits_{r\to\infty}r^{\frac{3}{2}}\pa{\frac{\partial}{\partial r}-ik_{j,\infty}}A_{j}\ne\infty\,.
            \end{equation}
            В дальнейшем удовлетворение этому условию следует из построения модового параболического уравнения и численной схемы его решения: учёт только исходящих волн по $x$ (см. \ref{sec::MPE}) и подавление отражения по $y$ (см. \ref{sec::boundary_conditions}).
        \subsection{Модовые параболические уравнения\label{sec::MPE}}
            \par Для получения параболических аппроксимаций однородный аналог уравнения горизонтальной рефракции \eqref{eq::HRE} представляется в виде
            \begin{equation}
            ...
            \end{equation}
        \subsection{Аппроксимация Паде\label{sec::pade}}
            ...
            \subsubsection{Аппроксимация оператора квадратного корня\label{sec::root_wampe}}
                ...
                \begin{equation}
                    \mathcal{A}_j^{n+1}=\frac{U_{l,m}\pa{L_j}}{W_{l,m}\pa{L_j}}\mathcal{A}_j^n\,,
                \end{equation}
                где
                \begin{alignat*}{3}
            ...
            \subsubsection{Метод аппроксимации Паде для пропагатора\label{sec::root_ssp}}
                ...
                \begin{equation}\label{eq::SSPADE}
                    e^{ik_{j,0}h\pa{\sqrt{1+L_j}-1}}\approx\frac{\tilde{U}_{l,m}\pa{L_j}}{\tilde{W_{l,m}}\pa{L_j}}=\tilde{a}_{l,m}^0+\sum\limits_{i=1}^p\frac{\tilde{a}_{l,m}^i}{1+\tilde{b}_{l,m}^iL_j}\,,
                \end{equation}
                получим
            ...
            \subsubsection{Коэффициенты аппроксимации Паде\label{sec::PADE}}
                ...
                \paragraph{Вычисление коэффициентов полиномов}
                    ...
                    \begin{equation}
                        ...
                        \end{alignedat}
                    \end{equation}
                    Таким образом, $\prescript{i}{}{\alpha}_{l,m}^F$ явно выражены через коэффициенты $\prescript{i}{}{\beta}_{l,m}^F$, которые могут быть найдены из решения правой системы уравнений.
                ...
                \par Коэффициенты разложения оператора квадратного корня\\
                    $ik_{j,0}\pa{\sqrt{1+L_j}-1}$ в ряд \eqref{eq::taylor} могут быть выражены формулой
                    \begin{equation}
                        c_0=0\,,\quad c_p=ik_{j,0}\frac{\pa{\frac{1}{2}}_p}{p!},\quad p=\overline{1,n}\,,
                    \end{equation}
                    где $\pa{x}_p=\prod\limits_{k=0}^{p-1}\pa{x-k}$ -- убывающий факториал.
                    \par Аналогично, коэффициенты разложения экспоненты $e^{ik_{j,0}h\pa{\sqrt{1+L}-1}}$ могут быть получены из рекуррентного соотношения
                    \begin{equation}
                        \begin{aligned}
                            c_0&=1\,,\\
                        ...
                        \end{aligned}
                    \end{equation}
        \subsection{Дискретизация оператора $L_j$\label{sec::lj}}
            ...
            \begin{equation}
                \underset{\alpha_{j,i}}{\underbrace{\frac{b_{l,m}^i}{k_{j,0}^2\delta^2}}}\mathcal{B}_{j,i}^{n+1,q-1}+\underset{\beta_{j,i}}{\underbrace{\pa{1+\frac{b_{l,m}^i}{k_{j,0}^2}\pa{k_j^2-k_{j,0}^2-\frac{2}{\delta^2}}}}}\mathcal{B}_{j,i}^{n+1,q}+\underset{\gamma_{j,i}}{\underbrace{\frac{b_{l,m}^i}{k_{j,0}^2\delta^2}}}\mathcal{B}_{j,i}^{n+1,q+1}=\mathcal{A}_j^{n,q}\,.\label{eq::numerical_scheme}
            \end{equation}
            Таким образом, коэффициенты $\mathcal{B}_{j,i}^{n+1,q}$ могут быть легко найдены обращением матриц с диагоналями $\alpha_{j,i},\beta_{j,i},\gamma_{j,i}$ методом прогонки \cite{abramov}. Уравнение \eqref{eq::numerical_scheme} представляет собой численную схему решения уравнения \eqref{eq::PDMPE}.
        \subsection{Граничные условия\label{sec::boundary_conditions}}
            \par Одной из особенностей МПУ является то, что их решение всегда рассмат­ривается в неограниченной области, в отличие от решения «обычных», или «вер­тикальных», параболических уравнений в подводной акустике, которое вычис­ляется в нескольких слоях, имеющих в качестве верхней границы поверхность океана и, как минимум в теории, некоторую границу на дне моря. Таким образом, искусственное ограничение вычислительной области является обязательным при численном решении МПУ. В рамках данной работы рассматривается два способа выполнения такого ограничения. В первом случае вычислительная область расширяется с обеих сторон с целью поглощения исходящих волн. Во втором, решение внутри области сопоставляется с решением, содержащим исходящие волны за её пределами, с использованием специальных искусственных граничных условий.
            \subsubsection{Согласованные поглощающие слои\label{sec::PML}}
                ...
                \end{equation}
                Таким образом, применимость PML граничных условий зависит от значения параметра $\varepsilon$, который должен быть существенно большим, чтобы сгладить исходящие волны, и функции $\beta\pa{y}$, которая в свою очередь должна существенно гладко достигать своего максимального значения при погружении в PML, так, при слишком быстром возрастании исходящие волны будут отражаться внутри поглощающего слоя.
            ...
            \subsubsection{Граничные условия прозрачности}
                ...
                \end{equation}
                Решение такой системы может быть найдено с использованием $\mathcal{Z}$-преобра\-зования (см. \niceref{sec::ztransform}{Главу}) по эволюционной координате $x$
                \begin{equation}
                ...
                \end{equation}
                Обозначим $\hat{\bm{\Psi}}_j^q=\pa{\hat{\mathcal{A}}_j^q,\hat{\mathcal{B}}_{j,1}^q,\dots,\hat{\mathcal{B}}_{j,p}^q}^T\in\mathbb{C}^{p+1}$, добавим к системе $\mathcal{Z}$-преобра\-зование уравнения \eqref{eq::BtoA}
                \begin{equation}
                ...
                и $\mathbf{P}_j^{-1}$ -- матрица левых собственных векторов вида
                \begin{equation}
                ...
                \begin{multline}
                    ...
                    \mathbf{P}_j^{-1}\pa{\mathbf{A}_j^{-1}\mathbf{B}_j+\mathbf{I}}\begin{pmatrix}
                        ...
                        \hat{\bm{\xi}}_j^q
                    \end{pmatrix}=\\
                    \mathbf{P}_j^{-1}\mathbf{P}_j\begin{pmatrix}
                        \mathbf{J}_j^1 & \mathbf{0}\\
                        \mathbf{0} & \mathbf{J}_j^2
                    \end{pmatrix}
                    \begin{pmatrix}
                ...
                \end{equation}
                где $\hat{\mathbf{D}}_j=-\pa{\mathbf{P}_j^4}^{-1}\mathbf{P}_j^3$. Применяя обратное $\mathcal{Z}$-преобразование \cite{lynn1986} получим дискретные правые прозрачные граничные условия
                \begin{equation}
                ...
                \end{equation}
                \paragraph{$\mathcal{Z}$-преобразование\label{sec::ztransform}}
                    \par Под $\mathcal{Z}$-преобразованием \cite{lynn1986} понимается свёртывание некоторой последовательности вещественных чисел во временной области в аналитическую функцию комплексной частоты. одностороннее $\mathcal{Z}$-преобразование последовательности $x_n$ при $n\geq 0$ задаётся как
                    \begin{equation}
                        X\pa{z}=\mathcal{Z}\left\{x_n\right\}=\sum\limits_{n=0}^\infty x_nz^{-n}\,.
                    \end{equation}
                    При этом соответствующее ему обратное преобразование может быть выражено как
                    \begin{equation}
                        x_n=\mathcal{Z}\left\{X\pa{z}\right\}=\frac{1}{2\pi i}\oint\limits_C X\pa{z}z^{n-1}\dd z\,,
                    \end{equation}
                    где $C$ -- контур, охватывающий область сходимости $X\pa{z}$
                    \begin{equation}
                        D=\left\{z\bigr|\lim\limits_{m\mapsto\infty}\sum_{n=0}^mx_nz^{-n}<\infty\right\}\,.
                    \end{equation}
                    \par Основными свойствами $\mathcal{Z}$-преобразования для области подводной акустики являются
                    \begin{subequations}
                        \begin{enumerate}
                            \item Линейность:
                                \begin{equation}
                                    \mathcal{Z}\left\{\alpha x_n+\beta y_n\right\}=\alpha\mathcal{Z}\left\{x_n\right\}+\beta\mathcal{Z}\left\{y_n\right\}\,,\quad\alpha,\beta\in\mathbb{R}
                                \end{equation}
                            \item Задержка по времени
                                \begin{equation}
                                    \mathcal{Z}\left\{x_{n-k}\right\}=z^{-k}\mathcal{Z}\left\{x_n\right\}\pa{z}\,,\quad k>0,x_n=0\forall n<0\,.
                                \end{equation}
                        \end{enumerate}
                    \end{subequations}
                    \par Дополнительно $\mathcal{Z}$-преобразование принято считать дискретным аналогом преобразования Лапласа
                    \begin{equation}
                        \mathcal{L}\pa{f}\pa{s}=\int\limits_{0}^{\infty}f\pa{t}e^{-st}\dd t\,.
                    \end{equation}
                При работе с дифференциальными уравнениями в частных производных преобразование Лапласа позволяет перейти к уравнению без частной производной по времени (или другой маршевой переменной). Являясь дискретным аналогом преобразования Лапласа, $\mathcal{Z}$-преобразование позволяет перейти к дискретному уравнению без численного дифференцирования по временной переменной.
        \subsection{Лучевая теория для уравнения горизонтальной рефракции\label{sec::ray_theory}}
            ...
            \subsubsection{Трассировка лучей, соответствующих вертикальным модам\label{sec::horizontal_rays}}
                \paragraph{Математическая постановка задачи}
                    ...
                    \begin{equation}\label{eq::ray_hamilton}
                        \begin{aligned}
                            \begin{dcases}
                        ...
                        \end{aligned}
                    \end{equation}
                    Каждое уравнение системы представляет собой обыкновенное дифференциальное уравнение, численное решение которых может быть выполнено с использованием методов Рунге-Кутты, описанных в следующей главе.
                \paragraph{Явные методы Рунге-Кутты}
                    ...
                    \par Пусть задана некоторая равномерная дискретная сетка\\ $\Omega=\left\{\pa{x_i,\boldsymbol{y}_i}\bigr|x_i=x_{i-1}+h_i,\boldsymbol{y}_i\approx\boldsymbol{y}\pa{x_i},x_i,h_i\in\mathbb{R},\boldsymbol{y}_i\in\mathbb{R}^n\right\}$, тогда явный $s$-ша\-говый метод Рунге-Кутты может быть записан в виде
                    \begin{equation}
                        \boldsymbol{y}_{i+1}=\boldsymbol{y}_i+h_i\sum\limits_{j=1}^{s}b_j\boldsymbol{k}_j
                    \end{equation}
                    где $\boldsymbol{k}_j$ значения функции $\boldsymbol{f}$, вычисленные в специальных промежуточных точках интервала
                    \begin{equation}
                        \begin{aligned}
        ...
        \subsection{Начальные условия}
            ...
            \subsubsection{Лучевые начальные условия}\label{sec::ray_starters}
                ...
                \begin{equation}
                    \mathcal{A}_j\pa{x_0,y}=M_j\pa{x_0,y}e^{ik_{j,0}S_j\pa{x_0,y}}+o\pa{\nicefrac{1}{k_{j,0}}}\,,
                \end{equation}
                ...
                \end{align}
                где $M_{j,0}=\nicefrac{e^{\nicefrac{i\pi}{4}}}{\sqrt{8\pi k_{j,0}}}$ --- амплитуда на расстоянии 1 м. от источника, $n_j\pa{l}=\nicefrac{k_j\pa{l}}{k_{j,0}}$ -- горизонтальный показатель преломления.
                \par Так как значения функций $S_j\pa{x_0,y}$ и $M_j\pa{x_0,y}$ вычисляются лишь на небольшом расстоянии $x_0$ (несколько десятков метров), в большинстве случаев будет достаточно начального условия рассчитанного для однородной среды при $k_j\pa{x,y}\equiv k_{j,0}$. В таком случае получим
                \begin{equation}\label{eq::ray_simple}
                    \begin{aligned}
                        S_j\pa{y}&=r\pa{y}\,,\\
                        M_j\pa{y}&=\frac{M_{j,0}}{r\pa{y}}\,,\\
        ...
        \subsection{Расчёт временных рядов в точках приёма при распространении импульсных акустических сигналов\label{sec::impulse}}
            ...
            \subsubsection{Преобразование Фурье}
                ...
                \begin{subequations}
                    ...
                    \end{enumerate}
                \end{subequations}
        \subsection{Уровень звукового воздействия\label{sec::SEL}}
            \par Уровень звукового воздействия (sound exposure level, SEL) в области $\Omega=\left\{\pa{x,y,z}\bigr|x_0\leqslant x\leqslant x_1,y_0\leqslant y\leqslant y_1,z_0\leqslant z\leqslant z_b\right\}$ для диапазона частот $\left[f_1,f_2\right]$ некоторого источника задаётся следующим интегралом
            ...
            \begin{equation}
                ...
                \end{gathered}
            \end{equation}
        \subsection{Колебательные скорость и ускорение\label{sec::particle_acceleration}}
            \par Колебательное ускорение $\mathbf{a}\pa{x,y,z}$ в области\\
            $\Omega=\left\{\pa{x,y,z}\bigr|x_0\leqslant x\leqslant x_1,y_0\leqslant y\leqslant y_1,z_0\leqslant z\leqslant z_b\right\}$ может быть вычислено через градиент акустического давления
            \begin{equation}
                \mathbf{a}=\left\{a_x,a_y,a_z\right\}=\left\{-\frac{1}{\rho}\frac{\partial p}{\partial x},-\frac{1}{\rho}\frac{\partial p}{\partial y},-\frac{1}{\rho}\frac{\partial p}{\partial z}\right\}=-\frac{1}{\rho}\nabla p\,.
            \end{equation}
            Используя модовое разложение \eqref{eq::MD} и извлекая главную осцилляцию $e^{ik_{j,0}x}$, запишем производные акустического давления по пространственным координатам
            \begin{align}
                &\begin{multlined}
                    \frac{\partial p}{\partial x}=\sum\limits_{i=1}^\infty\frac{\partial }{x}\pa{e^{ik_{j,0}x}\mathcal{A}_j\varphi_j}=\sum\limits_{i=1}^\infty e^{ik_{j,0}x}\pa{ik_{j,0}\mathcal{A}_j\varphi_j+\frac{\partial}{\partial x}\pa{\mathcal{A}_j\varphi_j}}=\\
                    \sum\limits_{i=1}^\infty e^{ik_{j,0}x}\pa{ik_{j,0}\mathcal{A}_j\varphi_j+\frac{\partial\mathcal{A}_j}{\partial x}\varphi_j+\frac{\partial\varphi_j}{\partial x}\mathcal{A}_j}\,,
                \end{multlined}\\
                &\frac{\partial p}{\partial y}=\sum\limits_{i=1}^\infty e^{ik_{j,0}x}\frac{\partial}{\partial y}\pa{\mathcal{A}_j\varphi_j}=\sum\limits_{i=1}^\infty e^{ik_{j,0}x}\pa{\frac{\partial\mathcal{A}_j}{\partial y}\varphi_j+\frac{\partial\varphi_j}{\partial y}\mathcal{A}_j}\,,\\
                &\frac{\partial p}{\partial z}=\sum\limits_{i=1}^\infty e^{ik_{j,0}x}\frac{\partial}{\partial z}\pa{\mathcal{A}_j\varphi_j}=\sum\limits_{i=1}^\infty e^{ik_{j,0}x}\mathcal{A}_j\frac{\partial\varphi_j}{\partial z}\,.
            \end{align}
            Удаление главной осцилляции позволяет перейти к производным по медленно изменяющимся функциям, что в свою очередь приводит к высокой численной стабильности при проведении численного дифференцирования даже простыми конечными разностями.
            \par Колебательная скорость $\mathbf{v}\pa{x,y,z}$ также может быть выражена через градиент акустического давления. Используя тот факт, что в частотной области $\mathbf{a}=-i\omega\mathbf{v}$ получим
            \begin{equation}
                \mathbf{v}=\left\{v_x,v_y,v_z\right\}=\left\{\frac{i}{\omega}a_x,\frac{i}{\omega}a_y,\frac{i}{\omega}a_z\right\}=-\frac{i}{\omega\rho}\nabla p\,.
            \end{equation}
    \newpage
    \subsection{Выводы ко второй главе}
        \par В данной главе диссертации рассматривается теория широкоугольных модовых параболических уравнений в адиабатическом приближении и предлагается новый способ их решения. В основе этого метода лежит применение аппроксимации Паде к пропагатору уравнения горизонтальной рефракции. Для искусственного ограничения вычислительной области выведены новые граничные условия прозрачности. Описывается лучевая теория для уравнения горизонтальной рефракции, на основе которой вводятся лучевые начальные условия, обладающие сколь угодно широкой апертурой в горизонтальной плоскости. Описывается вычисление временных рядов в точках приёма при распространении импульсных акустических сигналов, а также уровней звукового воздействия. Предлагается способ расчёта колебательных ускорений, вносящий лишь незначительную добавку в общее время вычислений.
